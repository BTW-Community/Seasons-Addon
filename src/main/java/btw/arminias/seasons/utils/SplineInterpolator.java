...
public class SplineInterpolator {
    ...
    public float[] approximationCache;
     ...
     * @param approximationCacheSize
     *            The amount of steps being saved in the approximation cache, accessible via {@link SplineInterpolator#getUnit(float)}
     * @return
     *
     * @throws IllegalArgumentException
     *             if the X or Y arrays are null, have different lengths or have fewer than 2 values.
     */
    public static SplineInterpolator createMonotoneCubicSpline(List<Float> x, List<Float> y, int approximationCacheSize) {
        ...
        SplineInterpolator sp = new SplineInterpolator(x, y, m);
        sp.buildCache(approximationCacheSize);
        return sp;
    }
    ...
    private void buildCache(int approximationCacheSize) {
        approximationCache = new float[approximationCacheSize];
        for (int i = 0; i < approximationCacheSize; i++) {
            approximationCache[i] = interpolate(minX + i * ((maxX - minX) / (approximationCacheSize)));
        }
    }

    public float get(float x) {
        float t = (MathHelper.clamp_float(x, minX, maxX - 0.00001F) - minX) / (maxX - minX);
        return approximationCache[(int) (t * approximationCache.length)];
    }

    public float getUnit(float x) {
        x = x % 1;
        return approximationCache[(int) (x * approximationCache.length)];
    }
}